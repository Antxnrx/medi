        ...
        response => {
          if (!response || !response.success) {
            ...
            if (response?.error === 'invalid_api_key') {
              errorMessage = "The Gemini API key is not configured. Please set a valid API key in the backend's .env file. You can get a Gemini API key from https://makersuite.google.com/app/apikey.";
              errorVerdict = "CONFIGURATION ERROR";
              errorDanger = "Moderate";
            } else if (response?.error === 'network_error') {
              errorMessage = "Network error connecting to the Gemini API. Please check your internet connection.";
              errorVerdict = "NETWORK ERROR";
              errorDanger = "Moderate";
            } else if (response?.error === 'parse_error') {
              errorMessage = "Failed to parse response from Gemini API. This might be a temporary issue.";
              errorVerdict = "API ERROR";
              errorDanger = "Moderate";
            }
          ...
          // Send scan data to background script for saving to Firestore
          chrome.runtime.sendMessage({
            action: 'saveScanData',
            data: {
              url: url,
            ...
            if (chrome.runtime.lastError) {
              console.error('Error sending scan data to background script:', chrome.runtime.lastError.message);
            } else if (response && response.success) {
              console.log('Scan data sent to background script successfully.');
            } else {
              console.error('Background script failed to save scan data:', response?.error || 'Unknown error');
            }
          });
