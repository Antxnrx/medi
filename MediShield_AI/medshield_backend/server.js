  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY || process.env.GEMINI_API_KEY === 'YOUR_GEMINI_API_KEY_HERE') {
      ...
      return res.status(500).json({ 
        ...
        message: "The Gemini API key is not configured. Please set a valid API key in the .env file. You can get a Gemini API key from https://makersuite.google.com/app/apikey."
      });
    }
...
`;
    ...
    if (err.code === 'ECONNREFUSED' || err.code === 'ENOTFOUND') {
      return res.status(503).json({ 
        error: "network_error", 
        detail: "Could not connect to Gemini API. Please check your internet connection.",
        original_error: err.message 
      });
    } else if (err.name === 'SyntaxError') {
      return res.status(500).json({ 
        error: "parse_error", 
        detail: "Failed to parse response from Gemini API.",
        original_error: err.message 
      });
    } else {
      res.status(500).json({ 
